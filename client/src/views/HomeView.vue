<template>
  ...
  <div class=" buttons">
    <button v-if="!isJoinRoomPopupVisible && !isCreateRoomPopupVisible" class="btn start-btn-create" @click="isCreateRoomPopupVisible = true">Create Room</button>
    <button v-if="!isJoinRoomPopupVisible && !isCreateRoomPopupVisible" class="btn start-btn-join" @click="isJoinRoomPopupVisible = true">Join Room</button>
 </div>
 <div class="create-room-popup mx-auto" v-if="isCreateRoomPopupVisible">
  <form @submit.prevent="onCreateRoomSubmit">
    <div class="form-group">
      <label for="roomName">Room Name</label>
      <input v-model="newRoom.name" id="roomName" class="form-control room-input" placeholder="Room Name" name="roomName" type="text">
    </div>
    <div class="form-group">
      <label for="password">Password</label>
      <input v-model="newRoom.password" id="password" class="form-control room-input" placeholder="Password" name="password" type="text">
    </div>
...
<div class="join-room-popup mx-auto" v-if="isJoinRoomPopupVisible && !isPasswordPopupVisible">
  ...
  <button class="btn join-room-popup-btn-close" v-if="!isPasswordPopupVisible" @click="isJoinRoomPopupVisible = false">Close</button>
  <br>
  <br>
  <br>
  <br>
...
<div class=" join-room-popup password-popup mx-auto" v-if="isPasswordPopupVisible">
  <form @submit.prevent="onJoinRoomSubmit">
    <div class="form-group">
      ...
      <input v-model="joinPassword" id="joinPassword" class="form-control room-input" placeholder="Password" name="joinPassword" type="text">
    </div>
...
.room-list-name {
  ...
  justify-content: space-between; /* Separate h1 and buttons */
}

.join-room-btns {
  display: flex;
  flex-direction: column-reverse; /* Stack buttons upside down */
...
.join-room-popup-btn-join,.start-btn-create,.room-input,.btn-create-room{
  background-color: #5C8374;
  color: #ffff;
}

.join-room-popup-btn-join:hover,.start-btn-create:hover,.btn-create-room:hover{
  background-color: #38574b;
  
}

.start-btn-join,.join-room-popup-btn-close,.join-room-btn-join,.btn-create-room-close{
  background-color: #183D3D;
  color: #ffff;
}
.start-btn-join:hover,.join-room-popup-btn-close:hover,.join-room-btn-join:hover,.btn-create-room-close:hover{
