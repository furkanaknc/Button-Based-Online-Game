  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", (roomId) => {
      ...
      if (rooms[roomId].players.length === 2) {
        ...
        io.to(rooms[roomId].players[rooms[roomId].currentPlayerIndex]).emit(
          "turn"
        );
      }
    });
    ...
    socket.on("damage", (data) => {
      ...
      const opponentSocketId = rooms[roomId].players.find(
        (id) => id !== socket.id
      );
      io.to(opponentSocketId).emit("damage-received", data.damage);
      sendLogMessageToRoom(
        roomId,
        `${userName} damaged the opponent for ${data.damage} points.`
      );
    ...
    socket.on("heal", (data) => {
      ...
      sendLogMessageToRoom(
        roomId,
        `${userName} healed for ${data.health} points.`
      );
      // Switch turn to the next player
      rooms[roomId].currentPlayerIndex = 1 - rooms[roomId].currentPlayerIndex;
      io.to(rooms[roomId].players[rooms[roomId].currentPlayerIndex]).emit(
    ...
    socket.on("random-damage", (data) => {
      ...
      sendLogMessageToRoom(
        roomId,
        `A Random Event!!! ${userName} inflicted random damage to the opponent for ${data.damage} points.`
      );
      // rooms[roomId].currentPlayerIndex = 1 - rooms[roomId].currentPlayerIndex;
    ...
    socket.on("random-heal", (data) => {
      ...
      sendLogMessageToRoom(
        roomId,
        `A Random event!!! ${userName} healed randomly for ${data.health} points.`
      );
      // rooms[roomId].currentPlayerIndex = 1 - rooms[roomId].currentPlayerIndex;
      // io.to(rooms[roomId].players[rooms[roomId].currentPlayerIndex]).emit('turn');
    });
